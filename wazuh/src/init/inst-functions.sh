...
InstallCommon()
{
  ...
  ${INSTALL} -d -m 1770 -o root -g ${OSSEC_GROUP} ${PREFIX}/tmp
    ...
    if [ ! -f ${PREFIX}/etc/weak_password ]; then
        ${INSTALL} -m 0640 -o root -g ${OSSEC_GROUP} ../etc/weak_password ${PREFIX}/etc/weak_password
    fi
...
InstallLocal()
{
    ...
    if [ ! -f ${PREFIX}/etc/lists/suspicious-programs ]; then
        ${INSTALL} -m 0660 -o ossec -g ${OSSEC_GROUP} -b ../etc/lists/suspicious-programs ${PREFIX}/etc/lists/suspicious-programs
    fi
