...
static int error_device = 0;
static int prev_device_id = 0;
static int error_account = 0;
static int prev_account_id = 0;
...
static int decode_device(Eventinfo *lf, cJSON * logJSON,int *socket);
static int decode_account( Eventinfo *lf, cJSON * logJSON,int *socket);
...
int DecodeSyscollector(Eventinfo *lf,int *socket)
{
    ...
    else if (strcmp(msg_type, "device") == 0 || strcmp(msg_type, "device_end") == 0) {
        if (decode_device(lf, logJSON, socket) < 0) {
            mdebug1("Unable to send dev information to Wazuh DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
    else if (strcmp(msg_type, "account") == 0 || strcmp(msg_type, "account_end") == 0) {
        if (decode_account(lf, logJSON, socket) < 0) {
            mdebug1("Unable to send accounts information to Wazuh DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
...
int decode_account(Eventinfo *lf, cJSON * logJSON,int *socket) {
    ...
    if (inventory = cJSON_GetObjectItem(logJSON, "account"), inventory) {
        if (error_account) {
            if (scan_id->valueint == prev_account_id) {
                retval = 0;
                goto end;
            } else {
                error_account = 0;
        ...
        cJSON * scan_time = cJSON_GetObjectItem(logJSON, "timestamp");
        cJSON * user_name = cJSON_GetObjectItem(inventory, "user_name");
        cJSON * user_id = cJSON_GetObjectItem(inventory, "user_id");
        cJSON * group_id = cJSON_GetObjectItem(inventory, "group_id");
        cJSON * user_info = cJSON_GetObjectItem(inventory, "user_info");

        cJSON * home_dir = cJSON_GetObjectItem(inventory, "home_dir");
        cJSON * shell_program = cJSON_GetObjectItem(inventory, "shell_program");
        cJSON * password = cJSON_GetObjectItem(inventory, "password");
        cJSON * weak_password = cJSON_GetObjectItem(inventory, "weak_password");
        cJSON * last_login_ip = cJSON_GetObjectItem(inventory, "last_login_ip");
        cJSON * last_login_time = cJSON_GetObjectItem(inventory, "last_login_time");


        snprintf(msg, OS_SIZE_6144 - 1, "agent %s account save", lf->agent_id);
        ...
        if (user_name) {
            wm_strcat(&msg, user_name->valuestring, '|');
            fillData(lf, "account.user_name", user_name->valuestring);
        } else {
            wm_strcat(&msg, "NULL", '|');
        }
        ...
        if (user_id) {
            char _pid[OS_SIZE_128];
            snprintf(_pid, OS_SIZE_128 - 1, "%d", user_id->valueint);
            fillData(lf,"account.user_id",_pid);
            wm_strcat(&msg, _pid, '|');
        } else {
            wm_strcat(&msg, "NULL", '|');
        }
        ...
        if (group_id) {
            char _ppid[OS_SIZE_128];
            snprintf(_ppid, OS_SIZE_128 - 1, "%d", group_id->valueint);
            fillData(lf,"account.group_id",_ppid);
            wm_strcat(&msg, _ppid, '|');
        } else {
            wm_strcat(&msg, "NULL", '|');
        }
        ...
        if (user_info) {
            wm_strcat(&msg, user_info->valuestring, '|');
            fillData(lf, "account.user_info", user_info->valuestring);
        } else {
            wm_strcat(&msg, "NULL", '|');
        }

        if (home_dir) {
            wm_strcat(&msg, home_dir->valuestring, '|');
            fillData(lf, "account.home_dir", home_dir->valuestring);
        } else {
            wm_strcat(&msg, "NULL", '|');
        }

        if (shell_program) {
            wm_strcat(&msg, shell_program->valuestring, '|');
            fillData(lf, "account.shell_program", shell_program->valuestring);
        } else {
            wm_strcat(&msg, "NULL", '|');
        }

        if (password) {
            wm_strcat(&msg, password->valuestring, '|');
            fillData(lf, "account.password", password->valuestring);
        } else {
            wm_strcat(&msg, "NULL", '|');
        }

        if (weak_password) {
            wm_strcat(&msg, weak_password->valuestring, '|');
            fillData(lf,"account.weak_password",weak_password->valuestring);
        } else {
            wm_strcat(&msg, "NULL", '|');
        }

        if (last_login_ip) {
            wm_strcat(&msg, last_login_ip->valuestring, '|');
            fillData(lf, "account.last_login_ip", last_login_ip->valuestring);
        } else {
            wm_strcat(&msg, "NULL", '|');
        }

        if (last_login_time) {
            wm_strcat(&msg, last_login_time->valuestring, '|');
            fillData(lf,"account.weak_password",last_login_time->valuestring);
        } else {
            wm_strcat(&msg, "NULL", '|');
        }
        ...
        if (wdbc_query_ex(socket, msg, response, OS_SIZE_6144) == 0) {
            if (wdbc_parse_result(response, &message) != WDBC_OK) {
                error_account = 1;
                prev_account_id = scan_id->valueint;
                goto end;
            }
        } else {
            error_account = 1;
        ...
        if (!msg_type) {
            merror("Invalid message. Type not found.");
            goto end;
        } else if (strcmp(msg_type, "account_end") == 0) {
            ...
            snprintf(msg, OS_SIZE_6144 - 1, "agent %s account del %d", lf->agent_id, scan_id->valueint);
...
int decode_device(Eventinfo *lf, cJSON * logJSON,int *socket) {
    ...
    if (inventory = cJSON_GetObjectItem(logJSON, "dev_inventory"), inventory) {
        if (error_device) {
            if (scan_id->valueint == prev_device_id) {
                retval = 0;
                goto end;
            } else {
                error_device = 0;
        ...
        cJSON * scan_time = cJSON_GetObjectItem(logJSON, "timestamp");
        cJSON * dev_type = cJSON_GetObjectItem(inventory, "dev_type");
        cJSON * dev_name = cJSON_GetObjectItem(inventory, "dev_name");
        cJSON * dev_id = cJSON_GetObjectItem(inventory, "dev_id");

        snprintf(msg, OS_SIZE_6144 - 1, "agent %s device save", lf->agent_id);
        ...
        if (dev_type) {
            wm_strcat(&msg, dev_type->valuestring, '|');
            fillData(lf,"device.dev_type",dev_type->valuestring);
        } else {
            wm_strcat(&msg, "NULL", '|');
        }

        if (dev_id) {
            wm_strcat(&msg, dev_id->valuestring, '|');
            fillData(lf,"device.dev_id",dev_id->valuestring);
        } else {
            wm_strcat(&msg, "NULL", '|');
        }

        if (dev_name) {
            wm_strcat(&msg, dev_name->valuestring, '|');
            fillData(lf,"device.dev_name",dev_name->valuestring);
        } else {
            wm_strcat(&msg, "NULL", '|');
        }
        ...
        if (wdbc_query_ex(socket, msg, response, OS_SIZE_6144) == 0) {
            if (wdbc_parse_result(response, &message) != WDBC_OK) {
                error_device = 1;
                prev_device_id = scan_id->valueint;
                goto end;
            }
        } else {
            error_device = 1;
        ...
        if (!msg_type) {
            merror("Invalid message. Type not found.");
            goto end;
        } else if (strcmp(msg_type, "device_end") == 0) {
            ...
            snprintf(msg, OS_SIZE_6144 - 1, "agent %s device del %d", lf->agent_id, scan_id->valueint);
