...
void* wm_sys_main(wm_sys_t *sys) {
    ...
    while (1) {
        ...
        /* Running account inventory */
        if (sys->flags.accountinfo){
            #if defined(__linux__)
                sys_account_linux(queue_fd, WM_SYS_LOCATION);
            #endif
        }
...
#if defined(__linux__)
        for (int i = 0; 1 < dev_scan_times; i++) {
            time_before = time(NULL);
            sys_device_linux(queue_fd, WM_SYS_LOCATION);
            time_after = time(NULL);
            int dev_internal = time_sleep * 100 - (time_after - time_before) * 1000;
            if (dev_internal > 0) {
                w_time_delay(dev_internal);
            }
        }
#else
...
void wm_sys_check() {
    ...
    if (!sys->flags.accountinfo) {
        mtdebug1(WM_SYS_LOGTAG, "Running accounts inventory disabled.");
    }
...
cJSON *wm_sys_dump(const wm_sys_t *sys) {
    ...
    if (sys->flags.accountinfo) cJSON_AddStringToObject(wm_sys,"accounts","yes"); else cJSON_AddStringToObject(wm_sys,"accounts","no");
#ifdef WIN32
