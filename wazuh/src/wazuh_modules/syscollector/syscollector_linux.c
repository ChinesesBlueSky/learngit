...
int check_weak_password(char *pwdp, char **password_dict);
...
void get_device_usb(int queue_fd, const char* LOCATION, int random_id, const char* timestamp){
    FILE *cmd_output;
    char buff[OS_SIZE_256];
    char *name;
    char *id;
    char *string;
    cmd_output = popen("lsusb |cut  -c 24-", "r");

    if (!cmd_output) {
        return;
    }
    while (fgets(buff,sizeof(buff) - 1, cmd_output)) {
        mtdebug2(WM_SYS_LOGTAG, "get_device_usb buff: '%s'", buff);
        id = strtok_r(buff, " ",&name);
        cJSON *object = cJSON_CreateObject();
        cJSON *dev_inventory = cJSON_CreateObject();
        cJSON_AddStringToObject(object, "type", "device");
        cJSON_AddNumberToObject(object, "ID", random_id);
        cJSON_AddStringToObject(object, "timestamp", timestamp);
        cJSON_AddItemToObject(object, "dev_inventory", dev_inventory);
        cJSON_AddStringToObject(dev_inventory, "dev_type", "usb");
        if (name) {
            cJSON_AddStringToObject(dev_inventory, "dev_name", name);
        }
        if (id) {
            cJSON_AddStringToObject(dev_inventory, "dev_id", id);
        }

        /* Send interface data in JSON format */
        string = cJSON_PrintUnformatted(object);
        mtdebug2(WM_SYS_LOGTAG, "get_device_usb sending '%s'", string);
        SendMSG(queue_fd, string, LOCATION, SYSCOLLECTOR_MQ);
        cJSON_Delete(object);
...
void get_device_bluetooth(int queue_fd, const char* LOCATION, int random_id, const char* timestamp){
    FILE *cmd_output;
    FILE *cmd_output2;
    char buff[OS_SIZE_256];
    char buff2[OS_SIZE_256];
    char *string;
    char cmd[OS_SIZE_128];
    cmd_output = popen("hcitool con |grep state |awk ‘{print $3}’", "r");
    ...
    while (fgets(buff,sizeof(buff) - 1, cmd_output)) {
        mtdebug2(WM_SYS_LOGTAG, "get_device_bluetooth buff: '%s'", buff);
        cJSON *object = cJSON_CreateObject();
        cJSON *dev_inventory = cJSON_CreateObject();
        cJSON_AddStringToObject(object, "type", "device");
        cJSON_AddNumberToObject(object, "ID", random_id);
        cJSON_AddStringToObject(object, "timestamp", timestamp);
        cJSON_AddItemToObject(object, "dev_inventory", dev_inventory);
        cJSON_AddStringToObject(dev_inventory, "dev_type", "bluetooth");
        cJSON_AddStringToObject(dev_inventory, "dev_id", buff);

        snprintf(cmd, sizeof(cmd), "hcitool name %s", buff);
        cmd_output2 = popen(cmd, "r");
        if (cmd_output2 && fgets(buff2, sizeof(buff2) - 1, cmd_output2)) {
            cJSON_AddStringToObject(dev_inventory, "dev_name", buff2);
            mtdebug2(WM_SYS_LOGTAG, "get_device_bluetooth buff2: '%s'", buff2);
        }
        pclose(cmd_output2);

        /* Send interface data in JSON format */
        string = cJSON_PrintUnformatted(object);
        mtdebug2(WM_SYS_LOGTAG, "get_device_bluetooth sending '%s'", string);
        SendMSG(queue_fd, string, LOCATION, SYSCOLLECTOR_MQ);
        cJSON_Delete(object);
...
void sys_device_linux(int queue_fd, const char* LOCATION){
    ...
    mtdebug1(WM_SYS_LOGTAG, "Starting Device inventory.");

    get_device_usb(queue_fd, LOCATION, random_id, timestamp);
    get_device_bluetooth(queue_fd, LOCATION, random_id, timestamp);

    /* Send interface data in JSON format */
    cJSON *object = cJSON_CreateObject();
    cJSON_AddStringToObject(object, "type", "device_end");
    cJSON_AddNumberToObject(object, "ID", random_id);
    cJSON_AddStringToObject(object, "timestamp", timestamp);
    string = cJSON_PrintUnformatted(object);
    mtdebug2(WM_SYS_LOGTAG, "sys_device_linux() sending '%s'", string);
    SendMSG(queue_fd, string, LOCATION, SYSCOLLECTOR_MQ);
    cJSON_Delete(object);
...
void sys_proc_linux(int queue_fd, const char* LOCATION) {
    ...
    mtdebug1(WM_SYS_LOGTAG, "Code updated!.");
...
void sys_account_linux(int queue_fd, const char* LOCATION) {
    char *timestamp = w_get_timestamp(time(NULL));
    // Define time to sleep between messages sent
    int usec = 1000000 / wm_max_eps;
    int random_id = os_random();
    ...
    mtdebug1(WM_SYS_LOGTAG, "Starting account information.");

    //load weak password dictionary once
    char **password_dict = NULL;
    FILE *fp = NULL;

    char dict_path[OS_FLSIZE + 1];
    if (isChroot()) {
        snprintf(dict_path, OS_FLSIZE, "%s", WEAK_PASSWORD_PATH);
    } else {
        snprintf(dict_path, OS_FLSIZE, "%s%s", DEFAULTDIR, WEAK_PASSWORD_PATH);
    }

    if(fp = fopen(dict_path, "rb"), !fp) {
        mterror(WM_SYS_LOGTAG, FOPEN_ERROR, dict_path, errno, strerror(errno));
        return;
    }

    password_dict = (char**)malloc(sizeof(char*)*OS_FLSIZE);
    for(int i = 0; i < OS_FLSIZE; i++) {
        password_dict[i] = (char*)malloc(sizeof(char)*OS_SIZE_32);
    }
    int i=-1;
    while( fscanf(fp, "%s", password_dict[++i]) != EOF ) {
        mtdebug1(WM_SYS_LOGTAG, "dictionary[%d] = %s\n",i, password_dict[i]);
    }
    password_dict[i] = NULL;
    w_fclose(fp);

    struct spwd *shadow_struct;
    struct passwd *pw;
    char *last_login_ip = NULL;
    char *last_login_time = NULL;
    char *save_ptr = NULL;
    char buff[OS_SIZE_256];
    FILE *cmd_output;
    char cmd[OS_SIZE_128] = {'\0'};
    int status;

    cJSON *account_array = cJSON_CreateArray();
    cJSON *item;
    char *string;
    ...
    while(pw = getpwent(), pw != NULL) {
        ...
        cJSON_AddStringToObject(object, "type", "account");
        cJSON_AddNumberToObject(object, "ID", random_id);
        cJSON_AddStringToObject(object, "timestamp", timestamp);
        cJSON_AddItemToObject(object, "account", account);
        cJSON_AddStringToObject(account, "user_name", pw->pw_name);
        cJSON_AddNumberToObject(account, "user_id", pw->pw_uid);
        cJSON_AddNumberToObject(account, "group_id", pw->pw_gid);
        cJSON_AddStringToObject(account, "user_info", pw->pw_gecos);
        cJSON_AddStringToObject(account, "home_dir", pw->pw_dir);
        cJSON_AddStringToObject(account, "shell_program", pw->pw_shell);

        if (strstr(pw->pw_shell, "/bin/bash")) {
            shadow_struct = getspnam(pw->pw_name);
            cJSON_AddStringToObject(account, "password", shadow_struct->sp_pwdp);

            mtdebug1(WM_SYS_LOGTAG, "hashed passwd = %s, username= %s, shell= %s\n", shadow_struct->sp_pwdp, pw->pw_name, pw->pw_shell);
            int check_weak_password_result = check_weak_password(shadow_struct->sp_pwdp, password_dict);
            cJSON_AddStringToObject(account, "weak_password", check_weak_password_result == 1 ? "yes":"no");

            // collect the last login ip and last login time for the account that is allowed to log in
            snprintf(cmd, OS_SIZE_128, "last --time-format iso | grep %s -m 1 | awk '{print $3,$4}'", pw->pw_name);
	    mtdebug1(WM_SYS_LOGTAG, "run command: %s", cmd);
            if (cmd_output = popen(cmd, "r"), cmd_output) {
                if(fgets(buff, sizeof(buff) - 1, cmd_output)){
	            last_login_ip = strdup(strtok_r(buff, " ", &save_ptr));
                    last_login_time = strdup(strtok_r(NULL, "\n", &save_ptr));
		    	
		    cJSON_AddStringToObject(account, "last_login_ip", last_login_ip);
                    cJSON_AddStringToObject(account, "last_login_time", last_login_time);
                }
               
		if (status = pclose(cmd_output), status) {
                    mtwarn(WM_SYS_LOGTAG, "Command '%s' returned %d.", cmd, status);
	        }
            } 
        }

        cJSON_AddItemToArray(account_array, object);
    }

    cJSON_ArrayForEach(item, account_array) {
        string = cJSON_PrintUnformatted(item);
        mtdebug1(WM_SYS_LOGTAG, "sys_account_linux() sending '%s'", string);
        wm_sendmsg(usec, queue_fd, string, LOCATION, SYSCOLLECTOR_MQ);
        free(string);
    }
    cJSON_Delete(account_array);
    os_free(last_login_ip);
    os_free(last_login_time);

    cJSON *object = cJSON_CreateObject();
    cJSON_AddStringToObject(object, "type", "account_end");
    cJSON_AddNumberToObject(object, "ID", random_id);
    cJSON_AddStringToObject(object, "timestamp", timestamp);

    char *end_msg;
    end_msg = cJSON_PrintUnformatted(object);
    mtdebug1(WM_SYS_LOGTAG, "sys_account_linux() sending '%s'", end_msg);
    wm_sendmsg(usec, queue_fd, end_msg, LOCATION, SYSCOLLECTOR_MQ);
    cJSON_Delete(object);
    free(end_msg);
    free(timestamp);
    free_strarray(password_dict);
}

//check the pwd is weak by dict attack? return 1 if weak, 0 if not.
int check_weak_password(char *pwdp, char **password_dict) {
    int retval = 0;
    char *salt = NULL;
    ...
    // parse salt
    const char *c = strrchr(pwdp, '$');
    if (c) {
        os_calloc(c-pwdp+1, sizeof(char), salt);
        memcpy(salt, pwdp, c-pwdp);
        salt[c-pwdp] = '\0';
 
        // dict attack
        for (int i=0; password_dict[i]; i++) {
            //crypt_r
            data4cry.initialized = 0;
            if( !strcmp(pwdp, crypt_r(password_dict[i], salt, &data4cry) ) ) {
                mtdebug1(WM_SYS_LOGTAG, "+++HIT+++! weak password = %s\n", password_dict[i]);
                retval = 1;
                break;
            }
        }
