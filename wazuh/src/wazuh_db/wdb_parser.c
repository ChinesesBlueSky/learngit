        ...
        *next++ = '\0';
        ...
        if (strcmp(query, "syscheck") == 0) {
            ...
            if (!next) {
                ...
                if (wdb_parse_device(wdb, next, output) == 0){
                    mdebug2("Updated 'sys_device' table for agent '%s'", sagent_id);
                } else {
                    merror("Unable to update 'sys_device' table for agent '%s'", sagent_id);
                }
            }
        ...
        } else if (strcmp(query, "account") == 0) {
            if (!next) {
                mdebug1("DB(%s) Invalid DB query syntax.", sagent_id);
                mdebug2("DB(%s) query error near: %s", sagent_id, query);
                snprintf(output, OS_MAXSTR + 1, "err Invalid DB query syntax, near '%.32s'", query);
                ...
                if (wdb_parse_accounts(wdb, next, output) == 0) {
                    mdebug2("Updated 'sys_accounts' table for agent '%s'", sagent_id);
                } else {
                    merror("Unable to update 'sys_accounts' table for agent '%s'", sagent_id);
                }
            }
...
int wdb_parse_device(wdb_t * wdb, char * input, char * output) {
    char * curr;
    char * next;
    char * scan_id;
    char * scan_time;
    ...
    if (next = strchr(input, ' '), !next) {
        mdebug1("Invalid device query syntax.");
        mdebug2("device query: %s", input);
        snprintf(output, OS_MAXSTR + 1, "err Invalid device query syntax, near '%.32s'", input);
        return -1;
    }
    ...
    *next++ = '\0';
    ...
    if (strcmp(curr, "save") == 0) {
        ...
        if (next = strchr(curr, '|'), !next) {
            mdebug1("Invalid device query syntax.");
            mdebug2("device query: %s", curr);
            snprintf(output, OS_MAXSTR + 1, "err Invalid device query syntax, near '%.32s'", curr);
            return -1;
        }
        ...
        *next++ = '\0';
        ...
        if (next = strchr(curr, '|'), !next) {
            mdebug1("Invalid device query syntax.");
            mdebug2("device query: %s", scan_time);
            snprintf(output, OS_MAXSTR + 1, "err Invalid device query syntax, near '%.32s'", scan_time);
            return -1;
        }
        ...
        *next++ = '\0';
        ...
        if (next = strchr(curr, '|'), !next) {
            mdebug1("Invalid device query syntax.");
            mdebug2("device query: %s", dev_type);
            snprintf(output, OS_MAXSTR + 1, "err Invalid device query syntax, near '%.32s'", dev_type);
            return -1;
        }

        if (!strcmp(dev_type, "NULL"))
            dev_type = NULL;

        dev_id = curr;
        *next++ = '\0';

        if (!strcmp(dev_id, "NULL"))
            dev_id = NULL;

        dev_name = next;
        if (!strcmp(dev_name, "NULL"))
            dev_name = NULL;

        if (result = wdb_device_save(wdb, scan_id, scan_time, dev_type, dev_id, dev_name), result < 0) {
            mdebug1("Cannot save device information.");
            snprintf(output, OS_MAXSTR + 1, "err Cannot save device information.");
        } else {
            snprintf(output, OS_MAXSTR + 1, "ok");
        }
        ...
        if (result = wdb_device_delete(wdb, scan_id), result < 0) {
            mdebug1("Cannot delete old device information.");
            snprintf(output, OS_MAXSTR + 1, "err Cannot delete old device information.");
        } else {
            snprintf(output, OS_MAXSTR + 1, "ok");
        }
...
int wdb_parse_accounts(wdb_t * wdb, char * input, char * output) {
    ...
    if (next = strchr(input, ' '), !next) {
        mdebug1("Invalid Account query syntax.");
        mdebug2("Account query: %s", input);
        snprintf(output, OS_MAXSTR + 1, "err Invalid Account query syntax, near '%.32s'", input);
        return -1;
    }
    ...
    *next++ = '\0';
    ...
    if (strcmp(curr, "save") == 0) {
        ...
        if (next = strchr(curr, '|'), !next) {
            mdebug1("Invalid Account query syntax.");
            mdebug2("Account query: %s", curr);
            snprintf(output, OS_MAXSTR + 1, "err Invalid Account query syntax, near '%.32s'", curr);
            return -1;
        }
        ...
        *next++ = '\0';
        ...
        if (next = strchr(curr, '|'), !next) {
            mdebug1("Invalid Account query syntax.");
            mdebug2("Account query: %s", scan_time);
            snprintf(output, OS_MAXSTR + 1, "err Invalid Account query syntax, near '%.32s'", scan_time);
            return -1;
        }
        ...
        *next++ = '\0';
        ...
        if (next = strchr(curr, '|'), !next) {
            mdebug1("Invalid Account query syntax.");
            mdebug2("Account query: %s", user_name);
            snprintf(output, OS_MAXSTR + 1, "err Invalid Account query syntax, near '%.32s'", user_name);
            return -1;
        }

        if (!strcmp(user_name, "NULL"))
            user_name = NULL;
        ...
        *next++ = '\0';
        ...
        if (next = strchr(curr, '|'), !next) {
            mdebug1("Invalid Account query syntax.");
            mdebug2("Account query: %s", user_info);
            snprintf(output, OS_MAXSTR + 1, "err Invalid Account query syntax, near '%.32s'", user_info);
            return -1;
        }

        if (!strcmp(user_info, "NULL"))
            user_info = NULL;
        ...
        *next++ = '\0';
        ...
        if (next = strchr(curr, '|'), !next) {
            mdebug1("Invalid Account query syntax.");
            mdebug2("Account query: %s", home_dir);
            snprintf(output, OS_MAXSTR + 1, "err Invalid Account query syntax, near '%.32s'", home_dir);
            return -1;
        }

        if (!strcmp(home_dir, "NULL"))
            home_dir = NULL;
        ...
        *next++ = '\0';
        ...
        if (next = strchr(curr, '|'), !next) {
            mdebug1("Invalid Account query syntax.");
            mdebug2("Account query: %s", shell_program);
            snprintf(output, OS_MAXSTR + 1, "err Invalid Account query syntax, near '%.32s'", shell_program);
            return -1;
        }

        if (!strcmp(shell_program, "NULL"))
            shell_program = NULL;
        ...
        *next++ = '\0';
        ...
        if (next = strchr(curr, '|'), !next) {
            mdebug1("Invalid Account query syntax.");
            mdebug2("Account query: %s", password);
            snprintf(output, OS_MAXSTR + 1, "err Invalid Account query syntax, near '%.32s'", password);
            return -1;
        }
        ...
        *next++ = '\0';
        ...
        if (next = strchr(curr, '|'), !next) {
            mdebug1("Invalid Account query syntax.");
            mdebug2("Account query: %s", weak_password);
            snprintf(output, OS_MAXSTR + 1, "err Invalid Account query syntax, near '%.32s'", weak_password);
            return -1;
        }

        if (!strcmp(weak_password, "NULL"))
            weak_password = NULL;
        
        last_login_ip = curr;
        *next++ = '\0';

        if (!strcmp(last_login_ip, "NULL"))
            last_login_ip = NULL;

        if (!strcmp(next, "NULL"))
            last_login_time = NULL;
        else
            last_login_time = next;

        // save to db
        if (result = wdb_account_save(wdb, scan_id, scan_time, user_name, uid, gid, user_info, home_dir, shell_program,
                                      password, weak_password, last_login_ip, last_login_time), result < 0) {
            mdebug1("wdb_parse_accounts(): Cannot save Account information.");
            snprintf(output, OS_MAXSTR + 1, "err Cannot save Account information.");
        } else {
            snprintf(output, OS_MAXSTR + 1, "ok");
        }
        ...
        if (result = wdb_account_delete(wdb, scan_id), result < 0) {
            mdebug1("Cannot delete old Account information.");
            snprintf(output, OS_MAXSTR + 1, "err Cannot delete old Account information.");
        } else {
            snprintf(output, OS_MAXSTR + 1, "ok");
        }
