...
// Function to save Network info into the DB. Return 0 on success or -1 on error.
int wdb_device_save(wdb_t * wdb, const char * scan_id, const char * scan_time, const char * dev_type, const char * dev_id, const char * dev_name)  {
...
int wdb_device_insert(wdb_t * wdb, const char * scan_id, const char * scan_time, const char * dev_type, const char * dev_id, const char * dev_name) {
    sqlite3_stmt *stmt = NULL;

    if (wdb_stmt_cache(wdb, WDB_STMT_DEVINFO_INSERT) < 0) {
        mdebug1("at wdb_device_insert(): cannot cache statement");
        return -1;
    }

    stmt = wdb->stmt[WDB_STMT_DEVINFO_INSERT];

    sqlite3_bind_text(stmt, 1, scan_id, -1, NULL);
    sqlite3_bind_text(stmt, 2, scan_time, -1, NULL);
    sqlite3_bind_text(stmt, 3, dev_type, -1, NULL);
    sqlite3_bind_text(stmt, 4, dev_id, -1, NULL);
    sqlite3_bind_text(stmt, 5, dev_name, -1, NULL);
    ...
    switch (sqlite3_step(stmt)) {
        ...
        case SQLITE_CONSTRAINT:
            if (!strncmp(sqlite3_errmsg(wdb->db), "UNIQUE", 6)) {
                mdebug1("at wdb_device_insert(): sqlite3_step(): %s", sqlite3_errmsg(wdb->db));
                return 0;
            } else {
                merror("at wdb_device_insert(): sqlite3_step(): %s", sqlite3_errmsg(wdb->db));
                return -1;
            }
        default:
            merror("at wdb_device_insert(): sqlite3_step(): %s", sqlite3_errmsg(wdb->db));
...
int wdb_device_delete(wdb_t * wdb, const char * scan_id) {
	sqlite3_stmt *stmt = NULL;

    if (!wdb->transaction && wdb_begin2(wdb) < 0){
        mdebug1("at wdb_device_delete(): cannot begin transaction");
        return -1;
    }

    if (wdb_stmt_cache(wdb, WDB_STMT_DEVINFO_DEL) < 0) {
        mdebug1("at wdb_device_delete(): cannot cache statement");
        return -1;
    }

    stmt = wdb->stmt[WDB_STMT_DEVINFO_DEL];

    sqlite3_bind_text(stmt, 1, scan_id, -1, NULL);

    if (sqlite3_step(stmt) != SQLITE_DONE) {
        merror("Deleting old information from 'sys_device' table: %s", sqlite3_errmsg(wdb->db));
        return -1;
    }
...
//added by wenqin
// Insert account info tuple. Return 0 on success or -1 on error.
int wdb_account_insert(wdb_t * wdb, const char * scan_id, const char * scan_time,
                       const char * user_name, int uid, int gid, const char * user_info,
                       const char * home_dir, const char * shell_program,
                       const char * password, const char * weak_password,
                       const char * last_login_ip, const char * last_login_time) {
    sqlite3_stmt *stmt = NULL;

    if (wdb_stmt_cache(wdb, WDB_STMT_ACCOUNT_INSERT) < 0) {
        mdebug1("at wdb_account_insert(): cannot cache statement");
        return -1;
    }

    stmt = wdb->stmt[WDB_STMT_ACCOUNT_INSERT];
    ...
    if (gid >= 0)
        ...
        sqlite3_bind_null(stmt, 5);
    sqlite3_bind_text(stmt, 6, user_info, -1, NULL);
    sqlite3_bind_text(stmt, 7, home_dir, -1, NULL);
    sqlite3_bind_text(stmt, 8, shell_program, -1, NULL);
    sqlite3_bind_text(stmt, 9, password, -1, NULL);
    sqlite3_bind_text(stmt, 10, weak_password, -1, NULL);
    sqlite3_bind_text(stmt, 11, last_login_ip, -1, NULL);
    sqlite3_bind_text(stmt, 12, last_login_time, -1, NULL);

    if (sqlite3_step(stmt) == SQLITE_DONE){
        return 0;
    }
    else {
        merror("at wdb_account_insert(): sqlite3_step(): %s", sqlite3_errmsg(wdb->db));
        return -1;
    }
}

// Function to save account info into the DB. Return 0 on success or -1 on error.
int wdb_account_save(wdb_t * wdb, const char * scan_id, const char * scan_time,
                     const char * user_name, int uid, int gid, const char * user_info,
                     const char * home_dir, const char * shell_program,
                     const char * password, const char * weak_password,
                     const char * last_login_ip, const char * last_login_time) {
    if (!wdb->transaction && wdb_begin2(wdb) < 0){
        mdebug1("at wdb_account_save(): cannot begin transaction");
        return -1;
    }
    return wdb_account_insert(wdb, scan_id, scan_time,
                           user_name, uid, gid, user_info, home_dir, shell_program, password, weak_password, last_login_ip, last_login_time);
}

// Delete account info about previous scan from DB.
int wdb_account_delete(wdb_t * wdb, const char * scan_id) {

    sqlite3_stmt *stmt = NULL;

    if (!wdb->transaction && wdb_begin2(wdb) < 0){
        mdebug1("at wdb_account_delete(): cannot begin transaction");
        return -1;
    }

    if (wdb_stmt_cache(wdb, WDB_STMT_ACCOUNT_DEL) < 0) {
        mdebug1("at wdb_account_delete(): cannot cache statement");
        return -1;
    }

    stmt = wdb->stmt[WDB_STMT_ACCOUNT_DEL];

    sqlite3_bind_text(stmt, 1, scan_id, -1, NULL);

    if (sqlite3_step(stmt) != SQLITE_DONE) {
        merror("Deleting old information from 'sys_accounts' table: %s", sqlite3_errmsg(wdb->db));
        return -1;
    }
