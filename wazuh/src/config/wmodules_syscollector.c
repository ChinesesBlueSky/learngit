...
static const char *XML_ACCOUNTS = "accounts";
...
int wm_sys_read(XML_NODE node, wmodule *module) {
    ...
    if(!module->data) {
        ...
        syscollector->flags.accountinfo = 1;
    ...
    for (i = 0; node[i]; i++) {
        ...
        } else if (!strcmp(node[i]->element, XML_ACCOUNTS)) {
            if (!strcmp(node[i]->content, "yes"))
                syscollector->flags.accountinfo = 1;
            else if (!strcmp(node[i]->content, "no"))
                syscollector->flags.accountinfo = 0;
            else {
                merror("Invalid content for tag '%s' at module '%s'.", XML_ACCOUNTS, WM_SYS_CONTEXT.name);
                return OS_INVALID;
            }
